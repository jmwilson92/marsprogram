...
void ACapeCampus::AddBelt(const FCapeBuilding& Building, float Z, float Height, ECapePaint Paint)
{
	...
	PaintBox(Paint, C + FVector(HalfX + 8.0f, 0.0f, Z), FVector(T, S.Y + 20.0f, Height));
	PaintBox(Paint, C + FVector(-(HalfX + 8.0f), 0.0f, Z), FVector(T, S.Y + 20.0f, Height));
	PaintBox(Paint, C + FVector(0.0f, HalfY + 8.0f, Z), FVector(S.X + 20.0f, T, Height));
	PaintBox(Paint, C + FVector(0.0f, -(HalfY + 8.0f), Z), FVector(S.X + 20.0f, T, Height));
}
	...
	int32 Cols, int32 Rows, float Z0, float Z1)
{
	...
	case ECapeDoorSide::PlusX:
		Place(C.X + S.X * 0.5f + 10.0f, true, S.Y);
		break;
	case ECapeDoorSide::MinusX:
		Place(C.X - S.X * 0.5f - 10.0f, true, S.Y);
		break;
	case ECapeDoorSide::PlusY:
		Place(C.Y + S.Y * 0.5f + 10.0f, false, S.X);
		break;
	...
	float Width, float Height, float MidZ)
{
	...
	{
	case ECapeDoorSide::PlusX:
		Origin = FVector(C.X + S.X * 0.5f + 18.0f, C.Y, MidZ);
		bAlongY = true;
		break;
	case ECapeDoorSide::MinusX:
		Origin = FVector(C.X - S.X * 0.5f - 18.0f, C.Y, MidZ);
		...
		break;
	case ECapeDoorSide::PlusY:
		Origin = FVector(C.X, C.Y + S.Y * 0.5f + 18.0f, MidZ);
		break;
	case ECapeDoorSide::MinusY:
		Origin = FVector(C.X, C.Y - S.Y * 0.5f - 18.0f, MidZ);
		break;
		...
		for (int32 Col = 0; Col < 4; ++Col)
		{
			...
			if (bAlongY)
			{
				PaintBox(ECapePaint::White,
					FVector(Origin.X + 8.0f, Origin.Y + Along, Z),
					FVector(10.0f, CantonW * 0.10f, CantonH * 0.08f));
			}
			else
			{
				PaintBox(ECapePaint::White,
					FVector(Origin.X + Along, Origin.Y + 8.0f, Z),
